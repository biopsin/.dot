...
local function updateOSD()
  local msg1 = fsize(52) .. 'Equalizer: ' .. ceq() .. iff(eq_enabled, 'On', 'Off')
  ...
  for i = 1, #bands do
    ...
    local info =
      ceq() .. fsize(52) .. data[1] .. ' hz ' .. fsize(52)
      .. iff(data[2] ~= 0 and eq_enabled, '', cdis()) .. data[2] .. ceq()
      .. fsize(52) .. ckeys() .. ' [' .. bands[i].keys[1] .. '/' .. bands[i].keys[2] .. ']'
      .. ceq() .. fsize(52) .. cnorm()
  ...
  local msg = ass('{\\an1}') .. msg3 .. nlb .. msg1 .. msg2
